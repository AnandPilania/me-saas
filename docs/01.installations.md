...
```shell
yarn add dayjs argon2 dotenv http-status-codes reflect-metadata class-validator cookie-parser compression hpp helmet
yarn add -D @types/node ts-node ts-node-dev @types/cookie-parser @types/compression
```
